...
<head>
    ...
    <title>Editar barrio | Dashboard</title>
...
<body>
    ...
    <div class="bg-fondo p-2 flex items-center justify-center h-screen bg-cover bg-center">
        ...
        <form action="{{ route('barrios.update', $barrio->id) }}" method="POST"
            ...
            {{-- Titulo --}}
            <h1 class="font-bold text-2xl text-center text-black">Editar barrio</h1>
            ...
            <x-input type="text" placeholder="Nombre del barrio" name="name" value="{{ old('name', $barrio->nombre)}}" />

            {{-- Enviar el formulario --}}
            <x-button type="submit">Confirmar cambios</x-button>
        </form>
    </div>
</body>
